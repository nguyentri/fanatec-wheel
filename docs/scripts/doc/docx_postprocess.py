...
class DocxPostProcessor:
            ...
            if tblGrid is not None:
                ...
                if n_cols > 0:
                    ...
                    if total_orig > 0:
                        ...
                        col_widths[-1] = max(0, content_w - sum(col_widths[:-1]))
                    else:
    ...
    def fix_table_style(
        ...
        if styles_bytes is not None:
            try:
                ...
                has_hdr = any(s.get(wtag("styleId")) == header_style for s in styles_tree.findall(wtag("style")))
                has_bdy = any(s.get(wtag("styleId")) == body_style for s in styles_tree.findall(wtag("style")))
                if not has_hdr or not has_bdy:
                    self._log(f"fix_table_style: WARNING - '{header_style}' or '{body_style}' not found in styles.xml. Word will fallback to Normal.")
    ...
    def run(
        ...
        steps = [
            ...
            ("remove_autogen_captions", lambda: self.fix_remove_autogen_captions(getattr(self, '_custom_caption_patterns', None))),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--remove-captions", nargs="+", metavar="REGEX",
        help="Additional regex patterns to remove as autogenerated captions.",
    )
...
def main() -> int:
    ...
    try:
        doc = DocxPostProcessor(source_docx, verbose=args.verbose)
        if args.remove_captions:
            doc._custom_caption_patterns = tuple(args.remove_captions)
